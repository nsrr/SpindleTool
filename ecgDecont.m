function cleansig=ecgDecont(sig,fs,ecg,fsecg)
...
if perc<30
    ...
    [cleansig,nois]=ekgsuppressionMODfilt(sig,ecg,fs,fsecg,R_t,0);
    am=rms(sig);
    c=1;
    rm=rms(nois)/am;
    while (rm>0.01 && c<10)
        [cclean,nois]=ekgsuppressionMODfilt(cleansig,ecg,fs,fsecg,R_t,0);
        if rms(nois)/am>=rm
            rm=0;
        else
            rm=rms(nois)/am;
